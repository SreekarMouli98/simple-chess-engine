...
describe('Game API', () => {
  ...
  test('loadBoard rejects invalid firstPlayer values', () => {
    ...
    expect(resp.success).toBe(false);
    expect(resp.reasons).toContain(
      "'firstPlayer' can either be 'black' or 'white'"
    );
  });

  test.todo(
    'getStatus reflects current player and check state after loadBoard'
  );
  ...
  test('getBoard rejects unsupported formats', () => {
    const game = new Game();
    expect(game.getBoard({ format: 'ARRAY' })).toBe(
  ...
  test('loadBoard rejects unsupported format', () => {
    ...
    const resp = game.loadBoard(content, {
      format: 'FEN',
      firstPlayer: 'white',
    });
